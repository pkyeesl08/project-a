...
7. [ElastiCache 생성 (Redis)](#7-elasticache-생성-redis)
...
| ElastiCache (cache.t3.micro) | Redis 캐시/소켓 | 월 750시간 무료 |
...
## 7. ElastiCache 생성 (Redis)

> WebSocket 매칭, 세션 캐시 등에 사용할 Redis를 만듭니다.
...
2. 좌측 메뉴 **"Redis OSS 캐시"** → **"Redis OSS 캐시 생성"**
3. 설정:
...
1. ElastiCache → Redis OSS 캐시 → `donggamerank-redis` 클릭
2. **기본 엔드포인트** 복사
