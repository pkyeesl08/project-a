...
| 학교 인증 | 이메일 또는 학생증 인증 |
| 27개 미니게임 | 반응/퍼즐/액션/정밀/특수 5개 카테고리 |
...
> ❌ 제거됨: 흔들기(자이로), 기울기 밸런스(자이로), 마이크 외치기(마이크) — 환경 제약으로 이탈 유발
> ✅ 추가됨: 순서대로 탭, 역방향 반응, 이중 정밀 탭, 역순 기억, 연속 조준 — 개인 능력 차이 극대화
...
11. **역순 기억** ⭐NEW — 5개 숫자를 보여준 뒤 역순으로 입력 (작업 기억 차이 극대화)

### 🎮 액션/모션
12. **방향 스와이프** — 화살표 방향으로 스와이프
13. **바 멈추기** — 이동하는 바를 정확한 위치에서 정지
14. **빠른 가위바위보** — AI를 이기는 패 선택
15. **순서대로 탭** ⭐NEW — 화면에 흩어진 1~7 숫자를 순서대로 최대한 빠르게 탭 (공간 기억+실행 속도)
16. **역방향 반응** ⭐NEW — 화살표가 나타나면 반대 방향을 탭 (인지 억제 능력 차이)

### 🎯 정밀/집중
17. **선 따라가기** — 곡선 경로를 손가락으로 추적
18. **타겟 저격** — 움직이는 타겟을 정확한 타이밍에 명중
19. **암실 탭** — 어둠 속에서 반짝이는 아이콘 탭
20. **나사 중심** — 회전하는 나사의 정중앙 탭
21. **선 늘리기** — 상대보다 긴 선 만들기
22. **이중 정밀 탭** ⭐NEW — 두 타겟을 순서대로 정중앙에 탭 (운동 정밀도 차이)
23. **연속 조준** ⭐NEW — 연속 등장하는 타겟 8개의 정중앙 탭 (에임 실력 차이 극대화)

### 🌟 특수/파티
24. **수학 속산** — 덧셈 문제 빠르게 풀기
25. **컵 게임** — 공이 숨겨진 컵 추적
26. **이모지 분류** — 이모지를 행복/슬픔으로 분류
27. **더 많이 세기** — 아이콘이 더 많은 그룹 선택
